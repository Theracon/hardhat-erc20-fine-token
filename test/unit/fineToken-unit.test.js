...
!developmentChains.includes(network.name)
  ? describe.skip
  : describe("FineToken Unit Test", function () {
      ...
      beforeEach(async function () {
        ...
        ourToken = await ethers.getContract("FineToken", deployer)
      })
      ...
      describe("allowances", () => {
        ...
        beforeEach(async () => {
          playerToken = await ethers.getContract("FineToken", user1)
        })
        it("Should approve other address to spend token", async () => {
          const tokensToSpend = ethers.utils.parseEther("5")
          await ourToken.approve(user1, tokensToSpend)
          const ourToken1 = await ethers.getContract("FineToken", user1)
